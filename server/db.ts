  ...
  if (!fs.existsSync(JSON_DB_PATH)) {
    ...
    try {
      ...
      if (data.products) {
        ...
        data.products = data.products.filter(p => p.id !== 1 && p.id !== 2 && p.id !== 3 && p.id !== 4);
...
async function bootstrapPostgresSchema() {
  ...
  try {
    ...
    // Active removal of pre-existing default product lines (1, 2, 3, 4) from PostgreSQL DB
    await client.query('DELETE FROM wishlists WHERE product_id IN (1, 2, 3, 4)');
    await client.query('DELETE FROM products WHERE id IN (1, 2, 3, 4)');
